 ...
 * COMP249 
 * Assignment 2
 * Written by Mateo Garzon 40277001 & Alessandro Lorigiano 40017756
 * 2024-03-27 Due
 * ------------------------------------------
 */

/*Movie class that extracts/parses movie records from csv files, 
 * sorts by category into ser files 
 * and puts movies into an array that can be navigated
 */
...
//				w.write("line:\t"+line +"\n");
	...
	/**
	 * Checks if the given array contains a specific target element.
	 *
	 * @param array  the array to search for the target element
	 * @param target the target element to search for in the array
	 * @return {@code true} if the target element is found in the array, {@code false} otherwise
	 */
	 ...
	 /**
	  * Reads the content of a specified file, categorizes records into genre-specific CSV files,
	  * and writes the genre filenames to a manifest file.
	  *
	  * @param file the path to the input file containing records to be categorized
	  * @throws IOException if an I/O error occurs while reading or writing files
	  */
	 ...
	 /**
	  * Reads the content of the original file, filters records by a specified category,
	  * and writes the filtered records to a new file with the specified category name.
	  *
	  * @param category the category to filter records by
	  * @param originalFile the path to the original file containing records to be filtered
	  */
	...
	/**
	 * Reads the content of a file containing file paths and returns an array
	 * containing these file paths.
	 *
	 * @param fileA the path to the file containing the list of file paths
	 * @return an array of file paths read from the input file
	...
	/**
	 * Processes the input file, categorizing its records into genre-specific files
	 * and generating a part2 manifest file. Additionally, it removes existing bad 
	 * records, part2 manifest, and genre-specific files if they exist, and then 
	 * proceeds to categorize the records into appropriate genre files.
	 *
	 * @param file the path to the input file containing records to be categorized
	 * @return the path to the generated part2 manifest file
	 */
	...
	/**
	 * Reads records from the input file, categorizes them into genre-specific arrays,
	 * and serializes each array into separate binary files. Additionally, it generates
	 * a part3 manifest file listing the names of the serialized files for each genre.
	 *
	 * @param file the path to the input file containing records to be processed
	 * @return the path to the generated part3 manifest file
	 */
	...
	/**
	 * Serializes an array of Movie objects into a binary file.
	 *
	 * @param movieArray the array of Movie objects to be serialized
	 * @param size       the number of valid elements in the movieArray
	 * @param outputFile the name of the binary file to serialize the movieArray into
	 */
	...
	/**
	 * Reads serialized Movie objects from files listed in the given manifest file and stores them in a 2D array.
	 *
	 * @param part3_manifest the path to the manifest file containing names of serialized Movie files
	 * @return a 2D array of Movie objects, where each row contains Movie objects from a single file
	 *         or null if an error occurs during reading or deserialization
	 */
	public static Movie[][] make_arrays(String part3_manifest) {
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                while (true) {
                    try {
                        ...
                        if (count == movies.length) {
                            ...
                            copyArray(movies, 0, newArray, 0, movies.length);
                            movies = newArray;
                        }
                        movies[count++] = movie;
                ...
                copyArray(movies, 0, trimmedMovies, 0, count);
